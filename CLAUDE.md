...
| **코드 작성 위임 — 난이도로 나눈다 (2026-09-09 사용자 지시)** | 쉬운 작업: 코덱스 테라-최대 · 복잡한 작업: 클로드 **오퍼스** 서브에이전트(`model: 'opus'`) | 2026-09-05의 코덱스 중단·소넷 대체는 이 지시로 해제됐다. 주문서 형식·절대 조건·자체 합격 판정 금지는 §1과 동일하게 적용한다. 무엇이 쉬운 작업인지는 계획 문서에 근거를 적는다([Task/FEATURE_PLAN_2026-09-09.md](Task/FEATURE_PLAN_2026-09-09.md) §1) |
...
**지침 (2026-09-08, 사용자 고정)**: 표본은 전부 로컬에 있다. **사용자에게 상태 확인을 요청하지 않는다.** 인식률이 낮은 이유는
클로드가 **자기 비전으로 이미지를 직접 보고** 분석한다 — 시스템이 자른 크롭(검수 화면이 보여 주는 것), 원본 스캔의 같은 자리,
촬영 이미지의 같은 자리를 **각각** 열어 비교한다. 스캔에는 보이는데 시스템 크롭에서 사라졌다면 보정 과정(빈 양식 차분·등록·이진화)이
지운 것이다. 정답표는 종이 기준이다: 연필·샤프·사인펜·상자 밖 표시는 스캔에서 안 보여도 정답표에는 값이 있다(`_visibility`).
...
```
p5  satisfaction.q01   빈칸이어야 한다   (2026-09-08부터 이것 하나)

# 2026-09-08 이전에 여기 있던 넷은 종이에 표시가 있다 — 스캔 1에서 안 보였을 뿐이다 (정답표 _visibility):
#   p19 basic.age = 14 (연필)  ·  p3 basic.gender = 여 (여자 글자의 여 위에 체크, 상자 밖)
#   p3 satisfaction.q01 = 3 (샤프, 흐림)  ·  p12 basic.gender = 남 (사인펜, 1비트에서 안 보임)
# 그 넷을 채점기가 채우면 이제 오답이 아니라 정답이다. 옛 기준선의 "빈칸인데 채움" 오답 중 여럿이 이것이었다.
```
...
있었다. **개수가 하나(p5 q01)가 아니면 이 문서가 아니라 정답표가 맞다**(2026-09-08, 사용자가 종이 기준으로 확정).
...
```
세트 1   CORRECT 360  WRONG 4  BLANK 73   OFF 30     (2026-09-08 정답표, 확정만 = RECOMMEND_BEST=0)
세트 2   CORRECT 351  WRONG 4  BLANK 82   OFF 16     (옛 정답표 348/7 — 오답 셋이 흐린 표시를 맞게 읽은 것이었다)
...
7. 학생을 순회하며 `확인 필요 항목 N개`와 항목 이름을 읽는다.
   ...
   **숨은 탭은 5분이 지나면 집중 스로틀링에 들어간다**(2026-09-07). 그때부터 `setTimeout`이 분당 1회가 되어,
   `await new Promise(r=>setTimeout(r,400))` 같은 짧은 대기 하나가 `Runtime.evaluate`를 45초에서 끊는다. 증상은
   "렌더러가 멈춘 것처럼 보이는데 `1+1`은 즉시 답하는" 것이다. 조치는 **탭을 새로 만드는 것**(타이머가 초기화된다)과
   **호출 안에서 기다리지 않는 것**이다 — 한 호출에 한 동작만 두면 왕복 시간이 대기 역할을 한다.
...
> **2026-09-07 계약 변경**([Task/RECOMMEND_BEST_2026-09-07.md](Task/RECOMMEND_BEST_2026-09-07.md)): 아래 표와 §3의 `WRONG = 0`은
> **게이트가 `high`로 확정한 값**에만 적용한다. 게이트가 확정하지 못한 그룹은 이제 빈칸이 아니라 **최고 점수 후보를 경합 추천으로**
> 넣는다(잉크가 아예 없을 때만 빈칸, `RECOMMEND_BEST=0`이 옛 동작). 추천 경로의 판정 잣대는 **추천 정밀도와 검수자 클릭 수**이지
> 오답 0이 아니며, 세트 네 개 재기·문턱 쓸기로 최적값을 찾는 라운드는 이 경로에 쓰지 않는다 — 사용자가 명시적으로 거부한 방식이다.
> **규칙 D**(2026-09-08, 같은 문서 §6): 추천은 거절 사유가 `none`·`absolute-floor`·`medium-path-not-offered`·`grid-unverified`·단독 `mark-shape`일 때만 낸다.
> `cancel-crossing`·`gap`·`relative-contrast`·`medium-gap`이 있으면 빈칸으로 남긴다 — 취소→다음 후보 규칙은 4/32였고, 경합 그룹은 14~40%였다.
> 네 세트 합산 정밀도 64% → **85%**(97/17).
> B-11 이후 자동값은 어차피 검수자 확인을 거치므로, "애매함 → 빈칸"은 클릭 한 번을 찾아서 타이핑하기로 바꾸는 손해였다.
...
- **기본정보의 절대 바닥을 내리는 것도 지렛대가 아니다**(2026-09-07, 같은 문서 §6). 바닥 계열만으로 거부된 칸은 네 세트에서 7개뿐이고
  **정답 5 · 오답 2**인데, 문턱을 0.014든 0.005든 어디에 두어도 같은 7개다 — 오답이 정답들 점수 한가운데 있다. 그리고 바닥을 조금이라도
  내리면 제일 먼저 들어오는 칸이 §3의 **빈칸이어야 하는 칸**(set1 p12 성별)이다. 두 갈래를 합치면 정답 33 · 오답 13 · 빈칸 채움 3
- **회색조 스캔은 같은 종이에서 신호를 1/3로 깎는다**(2026-09-07, 같은 문서 §7). 세트 1과 세트 4는 같은 학생·같은 종이를 설정만 바꿔 뜬 것인데
  기본정보 57칸의 채점기 점수 비가 **중앙값 0.339**(p10 0.065 · p90 0.908)다. 빈칸도 학교유형 6→9, 학년 3→7로 따라간다. 절대 바닥 0.021은
  `class=grayscale-scan`을 감지하고도 **계급별로 스케일되지 않지만**, 비율 분포가 너무 넓어 상수 보정도 성립하지 않는다.
  ~~**기본정보의 남은 지렛대는 코드가 아니라 스캔 설정이다**~~ **그것도 아니었다**(같은 날 저녁, 같은 문서 §8.1): 같은 9명을 흑백
  200dpi로 다시 뜨니 자동 입력은 145→158로 늘고 확인된 오답 1칸이 사라졌지만, **기본정보는 23/36으로 그대로**다(+13은 전부 표에서).
  운영 권고로는 **흑백 200dpi**가 낫다 — 파일도 19.7MB→437KB. 기본정보는 코드·문턱·입력 세 갈래가 모두 닫혔고 새 신호가 필요하다
- **기본정보 증거 게이트를 여는 것은 지렛대가 아니다**(2026-09-07, [Task/BASIC_BOX_GRAYSCALE_2026-09-07.md](Task/BASIC_BOX_GRAYSCALE_2026-09-07.md)).
  회색조에서 학교유형 9칸·학년 7칸이 비는데, 절반은 채점기가 통과시켰는데도 게이트(안쪽 8×8)가 버린 것이다. 네 세트를 모은
  그 모집단 39칸은 **정답 28 · 오답 11**이고, 오답 11에는 **빈칸이어야 하는 칸 2개**와 네 세트 모두에서 반복되는 취소 표시(p1 학년)가 들어 있다.
  `inner`로 부분 집합을 잘라도 오답 4개가 따라온다. 세트 4만 보면 상대 대비가 갈라 보이지만 네 세트를 모으면 완전히 겹친다(정답 1.02~57.60, 오답 1.15~4.93)
- **글리프 제외 잉크를 추천 경로의 폴백으로 넣는 것은 지렛대가 아니다**(2026-09-08, 같은 문서 §6). 프로브의 회색조 +26은 규칙 D가
  추천하지 않는 경합 그룹에 몰려 있어 제품 경로에서는 세트 1~3 동일, 세트 4 REC 36/8 → 35/9. 기각·되돌림. 값을 내려면 확정 경로(채점기)에
  들어가야 하고 그건 문턱 재보정과 한 묶음이다
- **빈 양식 차분이 스캔에 있는 ✓를 지운다**(2026-09-08, [Task/CONTAMINATION_2026-09-08.md](Task/CONTAMINATION_2026-09-08.md)). 선택지가 인쇄된 ⓪①②③④라
  ✓ 한 획의 잉크가 글리프의 10~20%인데, 빈 양식 자산의 글리프가 페이지 렌더보다 조금 진해 차분이 음수가 된다(정답 상자 page<blank).
  1비트 놓친 칸 중앙값 +0.004, 회색조는 정답 칸까지 −0.017(199/299 zeroed). 후보 151칸 중 눈으로 확인한 것은 전부 스캔·사진에
  또렷한 ✓였다(set1 p2 q01·q10, p6·p14 cagi q07). **문턱으로는 못 건진다** — 기준(글리프)이 다른 것이다. 다음 후보: 글리프 픽셀을
  뺀 영역만 비교
- **연필·샤프 표시는 스캔 과정에서 사라진다**(2026-09-08, 사용자 확인). 1비트에서는 거의 완전히, 회색조에서는 흐리게 남는다. 사인펜은 회색조에서
  살고 1비트에서 죽는다. 볼펜은 둘 다 산다. 정답표는 종이 기준이므로 이런 칸은 **채점기가 못 읽어도 빈칸(정상)이지 오답이 아니고**, 반대로 흐린
  신호를 읽어 낸 값은 정답이다 — 옛 기준선이 이런 칸을 "빈칸인데 채움" 오답으로 세던 것이 2026-09-08 정답표 교체로 바로잡혔다.
  스캔 입력의 한계이지 채점기 결함이 아니다. 운영 안내(볼펜/사인펜 사용)가 코드보다 먼저다
- **나이 크롭의 상단 인셋을 넓히는 것은 지렛대가 아니다**(2026-09-06, [Task/AGE_CROP_2026-09-06.md](Task/AGE_CROP_2026-09-06.md)).
  0.12가 숫자 윗부분을 자르는 것은 사실이고 0.06이 **네 세트 전부에서** 순수 판독을 개선한다(51/16 대 46/20). 그런데 제품 측정은
  반대로 간다 — 스캔 2·3과 사진 Set2에서 오답 +1씩. **신뢰도가 크롭에 대해 보정돼 있지 않아** 크롭을 넓히면 틀린 판독의 신뢰도도
  같이 올라가고(스캔2 p16: 0.41 → 1.00), 넓어진 크롭은 학년 범위 게이트가 없는 **tesseract에도 함께** 먹인다(스캔3 p10이 19).
  세트 1만 재면 인셋 0을 고르게 되는데 그것은 세트 2·3에서 더 나쁘다. 남은 갈래는 분류기와 tesseract의 크롭을 분리하는 것뿐이다.
  **브라우저는 중립이다** — 자동/정답/오답/빈칸이 358·352·6·74로 동일하고 셀 diff는 나이 두 칸이 서로 상쇄할 뿐이라(p7 잃고 p10 얻음), 노드 손해를 갚을 이득이 실사용 경로에 없다
