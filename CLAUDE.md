...
검수자는 표시 없는 칸에도 값을 지어내야 진행할 수 있었다(BUG_REPORTS §24). 두 번의 독립 맹독으로 10칸을
고쳤지만 빈칸 비율이 손 제작 구간의 1/3이라 **약 4칸이 아직 값으로 채워져 있을 수 있다.**
그러므로 **정답표와 어긋나는 칸은 종이를 보기 전까지 오답이 아니다** — 자세한 범위와 확인해야 할 19칸은
[Docs/18_GROUND_TRUTH_VALIDITY.md](Docs/18_GROUND_TRUTH_VALIDITY.md). p1~p6은 손 제작이라 이 노출이 없다.
