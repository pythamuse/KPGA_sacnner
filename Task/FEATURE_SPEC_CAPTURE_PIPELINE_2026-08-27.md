...
## §8 Stage A — 잔여 오답 6건의 기제 분류 (2026-08-27, 중앙 계측)

제품 이중 경로의 오답 6건을 `MARK_DECISION_TRACE`로 셀 단위 추적했다.

| 오답 | 추적에서 나온 것 | 기제 |
|---|---|---|
| p16 `satisfaction.q03~q05` (0↔1) | **`n=2`** — 5지선다에 후보 칸이 2개뿐인데 `outcome=high refused=none` | **A. 후보 집합 절단.** 격자가 행에서 칸을 2개만 찾았고, 게이트는 그 잘린 집합 안에서 통과시켰다. 남은 후보에 값이 재배치되므로 어떤 선택이든 값 매핑이 무의미하다 |
| p4·p5 `satisfaction.q10` (2↔4) | 학생은 ④ 체크(크롭 확인), 승자 상자 `shift=35~47px`, **`cells=1`** | **B. 부분 격자로 오염된 좌표계.** 5지선다에 격자 셀 1개 — 나머지는 템플릿 폴백과 뒤섞이고, 정렬 탐색이 상자를 열 간격의 절반 가까이 밀어 이웃 열의 잉크를 집는다. A와 같은 과: **불완전한 격자 구조가 통과한 것** |
| p15 `basic.age` (17↔15) | 게이트 경로 아님 | **C. 숫자 OCR 오독.** 나이 필드의 tesseract 경로 — 별개 기제 |

**여섯 중 다섯이 한 원인이다: 격자 검출이 choice 수보다 적은 칸을 돌려줘도 그대로
쓰인다.** 스캔에서는 격자가 거의 항상 완전해 이 구멍이 보이지 않았고, 사진의 조각난
격자가 처음으로 이 경로를 오답으로 만들었다.

**수정 방향(다음 사이클, §5.4-안전):** 필드의 격자 override 칸 수가 그 필드의 choice 수와
다르면 **override를 버리고 템플릿 rect로 폴백하거나 그 필드를 거절**한다 — 통과 기준을
낮추는 것이 아니라 구조 검증을 추가하는 조임이므로 §5.4에 걸리지 않는다. 판정 측정:
19명 전수에서 `WRONG` 감소, 미확정 증가 허용.
