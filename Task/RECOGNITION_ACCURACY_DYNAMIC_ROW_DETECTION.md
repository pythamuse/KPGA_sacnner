...
## 사이클 1 — 1차 탐색 (2026-08-03 초, 실제 오정렬 증거 확보 전)
...
권장: **A(자체 처리)**. 사용자가 **"자체 처리(권장)"**를 선택.

### 테스트 결과

해당 없음 — 방식 검토/제안 단계, 코드 변경 없음. 이 시점에는 정확한 실패 지점(좌표 오정렬 vs 신호 약함)이 픽셀 단위로 확정되지 않은 상태였다.

### 다음 작업을 위한 피드백

실제 ROI 디버그 스크린샷을 확보한 뒤 구체적 설계를 이어가기로 함 — 사이클 2.

## 사이클 2 — 실제 근본 원인 확인 및 방법 재검토 (2026-08-03, 실 ROI 스크린샷 확보 후)

### 작업 내용

[[MOBILE_CAPTURE_PERSPECTIVE_CORRECTION]] 사이클 3에서 확인된 대로, 문제는 "신호 약함"이 아니라 **페이지 경계 추정 실패로 인한 좌표 전체 밀림**임이 확인됨. 이 root cause를 반영해 방법을 재검토:
...
해당 없음 — 방법 결정 단계, 코드 변경 없음.

### 다음 작업을 위한 피드백

이 방법을 작업지시서로 구체화해 Codex에 구현 위임 → 사이클 3.

## 사이클 3 — 작업지시서 작성, Codex 구현, 자체 코드 리뷰

### 작업 내용

작업지시서(당시 `Docs/34_TABLE_ROW_DETECTION_WORK_ORDER.md`) 핵심 요구사항(코덱스에 위임 시 명시):
...
**자체 코드 리뷰**: 작업지시서 요구사항을 충실히 따랐음을 확인. 다만 리뷰 중 설계 리스크 발견: `buildCagiRowOverrides`/`buildSatisfactionGroupOverrides`가 감지한 가로선을 `roiTemplates.ts`의 문항 **행 중심(row-center)** Y좌표에서 유도한 간격 패턴과 매칭하는데, 실제 촬영된 설문지에서 안정적으로 감지 가능한 가로선은 대개 표의 **행 경계(구분선)**이지 행 중심이 아닐 가능성이 높다는 것.

### 테스트 결과

- Codex 자체 보고 기준 `npm test`/`npm run build` 통과, 신규 6개 테스트 포함.
- 내 코드 리뷰: 작업지시서의 안전/구조 요구사항(1~4번)은 전부 충족 확인. 다만 "행 중심 vs 행 경계" 설계 리스크는 코드 리뷰만으로는 실제 영향도를 알 수 없음 — 실증 검증 필요.

### 다음 작업을 위한 피드백

행 중심/행 경계 리스크를 synthetic 이미지로 직접 검증 → 사이클 4.

## 사이클 4 — 실증 검증 (synthetic 이미지 기반)
...
검증 결과를 반영해 방어적 완화 조치: `buildOverridesFromRowCenters`의 크롭 여백 배수를 `interval * 0.3` → `interval * 0.45`로 확대(매칭 성공 시 중심에서 다소 벗어난 실제 마크 위치까지 크롭 범위 포함). `tests/table-row-detection.test.ts`의 하드코딩 기대값 3개(`cagi.q01`/`q08`/`q09`) 갱신.
...
- 여백 확대 후 `npm test`: 11개 파일, 43개 테스트 전부 통과(신규 `table-row-detection.test.ts` 6개 포함). `npm run build`: 정상 컴파일/타입체크/정적 생성 완료, 에러 없음.
...
- **다음 단계**: 실제 사용자의 촬영/업로드 이미지로 재검증 필요. 가능하다면 오정렬이 확인됐던 실제 ROI 사례(사이클 2 근거 이미지)로 `buildCagiRowOverrides`가 오버라이드를 실제로 생성하는지, 생성한다면 올바른 위치인지 직접 확인하는 것이 가장 확실한 검증 방법.
