# B-11 — 기계 입력의 확정 계산 제거와 미확정 저장 게이트 (2026-09-03)

감사 소견 B-11([Docs/16](../Docs/16_RECOGNITION_DESIGN_AUDIT.md) §D 1순위)과 검증 기록
[§3.11](DESIGN_AUDIT_VERIFICATION_2026-09-03.md)의 수정. 코덱스(테라-최대) 위임, 브랜치
`codex-b11-settlement`, 커밋 `ddaff0b`. 아래 §0은 위임자의 판정, 그 뒤는 코덱스의 보고서 원문이다.

## 0. 판정 — 병합

값 경로 무변경(`src/lib/recognition`·`labelExport`·`excel` diff 없음)이므로 §5.4의 정확도 판정은
해당 없고, 렌더 시험과 브라우저 실사용으로 판정했다. 노드 428/0, tsc 통과(위임자 재실행).

**브라우저(세트 1, 19명, dev 서버, 2026-09-03)**

| 단계 | 관측 |
|---|---|
| 인식 직후 학생 1 | 확인 필요 23개(자동 18 + 낮음 2 + 권장 1 + 빈칸), 경고 "확인되지 않은 자동 입력 18개", **저장 버튼 비활성** |
| 서버 거절 | `POST /api/students`에 `auto`·`restored` 출처 → **400** `fields: [cagi.q01, cagi.q02]`(`confirmed`는 제외됨) |
| 확인 후 저장 | "이 값이 맞음" 17회 + 빈칸 5개 수기 입력 → 저장 성공, "작성 완료된 학생 데이터 (1명)" |
| 다른 학생 갔다 복귀 | 경고 없음, 배지 **확인 완료 18 · 수기 수정 5**, 버튼 "수정 내용 반영 (엑셀 3행)" |
| 새로고침 → 이어서 하기 | 동일(출처가 저장 행과 스냅샷을 거쳐 살아남음) |
| 저장 실패 1건 | 빈칸을 `blank_ok`로 두고 저장 → 기존 `validateStudent`가 5건 거절. **B-11과 무관한 기존 동작**(보고서 §"빈값") |

**부수 관측**
- 검수 작업량이 늘었다. 이전에는 high 자동값이 주의 목록 밖이었으나 이제 학생당 ≈18칸을 "이 값이
  맞음"으로 눌러야 저장된다. 이것은 B-11의 **의도된 결과**이고 §5.4의 "확인 필요 합계" 기준선(135)은
  **의미가 바뀌어 재설정이 필요하다** — 다음 19명 전수에서 새 기준선을 잡는다. 일괄 확인 버튼은
  만들지 않았다(사용자 결정 사항, 아래).
- dev 서버 로그에 `Cannot find module .next/worker-script/node/index.js`(tesseract.js 워커) 예외가 인식
  중 반복됐다. 인식 코드는 이 라운드에서 변경되지 않았으므로 기존 문제다. **브라우저에서 나이가 19/19
  빈칸인 것이 이 실패 때문일 수 있다** — Docs/17 §3.6의 tesseract 재판정에 주의 사항으로 추가했다.
  → 같은 날 확인·수정: BUG #23, `workerPath` 명시. 프로덕션 빌드에서 세트 1 나이 2칸 정답.

**사용자 결정 대기**: 학생당 18회 클릭을 줄이는 "자동값 일괄 확인"을 둘 것인가. 둔다면 오답 0의
관문을 다시 약하게 만들므로, 최소한 크롭을 한 번 스크롤해 본 뒤에만 활성화되는 형태여야 한다.

---

# (코덱스 보고서 원문) B-11 저장 출처·settlement 구현 보고서
