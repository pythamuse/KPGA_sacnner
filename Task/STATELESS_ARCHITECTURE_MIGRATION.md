...
### 1. 최초 발견 (2026-08-01, PR #2 Vercel 프리뷰 실기 테스트)

실제 Chrome으로 배포본(`.../pull/2` 프리뷰)에 접속해 청소년/성인 두 트랙을 새 작업 생성 → 업로드 → 인식 → 검수 → 저장 → 다운로드까지 직접 실행. UI/트랙 분기 로직 자체는 정상 동작했으나, 다음 두 문제를 재현:
...
### 2. 재확인 및 방향 결정 (2026-08-02, PR #3 Vercel 프리뷰)
...
같은 근본 원인(`/api/uploads/crop`, `/api/uploads/image`가 로컬 디스크 파일에 의존)이 검수 화면의 크롭/원본 미리보기에도 있었음.
...
### 5. 부작용: data URI 새 탭 열기 차단 (2026-08-03)
...
(별도 검증 기록 없음 — 이 문서에는 수정 방향까지만 기록되어 있었음. 원본 Docs/25에 검증 결과 섹션이 없었음.)

## 다음 작업을 위한 피드백

- 저장/다운로드/이미지 미리보기 3개 모두, Vercel 다중 인스턴스 환경 자체는 로컬 dev 서버로 재현 불가능하므로 "코드가 요청 간 서버 로컬 상태에 의존하지 않는다"는 것을 코드 리뷰 + 로컬 E2E로 확인한 것이지, 실제 다중 인스턴스 환경에서의 최종 확인은 아니다. 배포 후 실사용 환경에서 저장→다운로드까지 한 번 더 확인 권장.
- 성인 트랙 "연령대" crop 미리보기가 청소년 좌표를 쓰는 문제([Docs/14] 3.2절)는 이번 무상태화 작업 범위에서 다루지 않았다 — 성인 ROI 좌표가 정의되기 전까지는 `track === 'adult'`일 때 `basic.age` crop 미리보기 자체를 숨기는 처리가 필요.
- JBIG2 PDF 렌더링 이슈([[PDF_BATCH_RENDER_HANG]])는 이번 작업과 무관한 별개 문제로, 계속 미해결 상태.
- `data:` URI 새 탭 차단 수정은 검증 기록이 없다 — 재배포 후 실제로 크롭/원본 이미지 링크가 새 탭에서 열리는지 한 번 확인 필요.
