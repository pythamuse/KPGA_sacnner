...
## 사이클 1 — pdf.js 6.1.200(ESM) 업그레이드

### 작업 내용

**원인 조사**: 실제 Chrome에서 `src/components/ImageUploadPanel.tsx`의 `convertPdfToImages()`/`renderPdfPageToFile()`이 호출하는 pdf.js 단계를 하나씩 직접 실행하며 타임아웃을 걸어 확인:
...
`getOperatorList()`는 빠른데 `render()`만 멈춘다는 것은 PDF 구조 파싱은 문제없고, 임베딩된 이미지를 디코딩/래스터화하는 단계에서 멈춘다는 뜻이다. 스캐너 소프트웨어가 흑백 스캔 문서에 흔히 쓰는 CCITT G4/JBIG2류 압축 이미지를, 이 앱이 CDN으로 고정해 쓰던 pdf.js `3.4.120`(2023년 빌드)이 처리하다 멈추는 유형으로 추정. 기존 자동 테스트는 전부 합성 PNG/SVG 이미지를 쓰고 실제 브라우저의 pdf.js 렌더링 경로를 통과한 적이 없어 이 문제를 못 잡았다.

**후보 비교**:
- **후보 A — 클라이언트 pdf.js 버전/로딩 방식 교체**: 변경 범위 작음, 서버 의존성 불필요. 다만 v4+ 부터는 UMD 빌드가 없어 ES 모듈(`.mjs`) 전환이 필요.
- **후보 B — PDF→이미지 변환을 서버로 이전**: poppler/mupdf/pdfium 계열이 스캔 이미지 코덱을 더 안정적으로 지원하지만, Vercel 서버리스에서의 검증이 필요하고 변경 범위가 큼.

→ 후보 A부터 시도, 실패 시 B로 전환하는 방향으로 결정.

**구현**: pdf.js 3.4.120 → 3.11.174(3.x 최신, UMD 유지)로 먼저 시도 → **실패**(동일하게 20초 이상 멈춤, 3.x 라인 안에서는 해결 안 됨). → 6.1.200(ESM 전환)으로 재시도:
...
- 이 시점에는 "38페이지 전부 성공"으로 결론 내렸으나, **이 테스트는 실제로는 1페이지만 반복 실행한 것**이었다 — 표본이 편향되어 있었다는 게 사이클 2에서 드러난다. 앞으로 pdf.js 렌더링을 검증할 때는 반드시 서로 다른 여러 페이지를 각각 단독으로 테스트할 것.

## 사이클 2 — JBIG2 잔존 버그 재발견

### 작업 내용

2026-08-02, PR #3의 Vercel 프리뷰를 실제로 재현 테스트하던 중, 같은 파일의 **6페이지를 단독으로 렌더링**하면:
...
즉 pdf.js 6.1.200으로도 "PDF 렌더링 멈춤"이 전부 해결된 게 아니라, 사이클 1에서는 우연히 1페이지가 JBIG2를 안 쓰는 페이지라 통과했을 뿐이고, JBIG2로 인코딩된 페이지는 여전히 멈춘다. pdf.js는 순수 JS 자체 JBIG2 디코더를 쓰는데, 특정 스캐너가 만드는 JBIG2 인코딩 방식에서 취약점이 있는 것으로 보인다.
...
별도 조치: 근본 해결과 무관하게, 한 페이지가 멈춰도 전체 배치가 무기한 멈추지 않도록 `src/lib/pdf/withTimeout.ts` 기반 페이지별 타임아웃 방어 코드를 추가.

### 테스트 결과

- 같은 파일의 6페이지 단독 렌더링 시 50초 이상 무응답 재현 확인.
- 사이클 1의 "38페이지 전부 성공" 결론은 **정정 필요** — 1페이지만 반복 검증한 표본 오류였음.
- `withTimeout.ts` 적용 후: 최소한 "영원히 멈춘 것처럼 보이는" 상황은 20초 후 명확한 오류로 바뀜을 확인. 다만 이는 증상 완화이지 근본 해결이 아니다.
