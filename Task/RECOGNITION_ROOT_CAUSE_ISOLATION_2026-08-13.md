...
> **2026-08-19 — 이 절 이하의 모든 숫자는 무효다.** 계측기가 `scale 2.0` PNG 무손실로 렌더해 왔는데, 배포본은 `scale 1.5` JPEG로 렌더해 업로드한다. 같은 커밋·같은 PDF에서 두 입력의 결과가 `CORRECT 116/135, WRONG 0`(계측기) 대 `CORRECT 92/135, WRONG 1`(배포본)으로 갈렸다. 따라서 사이클 0~10의 판정은 **배포본과 무관한 입력에서 내려진 것**이며, 병합된 네 라운드가 실제로 개선이었는지는 아직 모른다. 새 기준선과 재판정 계획은 [[MEASUREMENT_RENDER_PARITY_2026-08-19]] 참고.
...
| 2 | ~~`exp/cause-a-checkbox-detection`~~ | A 체크박스 직접 검출 | CORRECT +16, WRONG 0 | **병합 완료** (`main`, v2026-08-14.1). 4번째 시도에 성공. 브랜치 삭제 |
...
| 4b | ~~`exp/cause-c1b-rejected-grid-fallback`~~ | C1-b 거부된 격자의 행 폴백 | — | **종결: 결함 아님.** 안전장치의 정상 동작. 브랜치 삭제 |
...
### 사이클 10 — 원인 A 체크박스 직접 검출 (코덱스) (2026-08-14)

**방향과 설계**

방향 2(승인 등급 부여)는 최종 기각된 상태이므로, 이번 사이클은 방향 1인 체크박스 자체 검출만 구현했다. `templateBaseline.ts`가 한 번 로드하는 빈 CAGI 양식에서 기본정보 체크박스를 먼저 검출하고, 다음 절차로 런타임 스캔과 정합했다.

1. 빈 양식에서 8-연결 어두운 성분을 검출하고 체크박스 크기·종횡비·채움비 및 사각 프레임 점수를 적용했다. 빈 양식의 5개 행 배치에서 가장 높은 점수의 12개를 선택하여 성별 2, 학교유형 4, 학년 6의 기준 배치를 만들었다. 결과는 12/12개이며 중복 사각형이 없었다.
2. 빈 양식과 실제 스캔 모두 `contentBounds` 기준 정규화 좌표를 사용했다. 실제 렌더의 체크 표시가 사각형 테두리와 연결되어 단일 성분 필터에서 빠지는 경우가 있어, 실제 스캔에는 예상 박스 크기의 프레임 윈도우를 3px 간격으로 추가 탐색했다. 이는 특정 필드나 후보 인덱스의 예외가 아니라 전체 기본정보 체크박스 집합에 동일하게 적용된다.
3. 기준 12개 중심과 실제 후보 중심에 대해 `dx, dy` 평행이동을 모든 기준-후보 쌍에서 시도하고, 허용 오차 안의 일대일 완전 매칭 중 잔차가 가장 작은 것을 선택했다. 최대 이동은 정규화 좌표 0.03, 후보 허용 오차는 0.018이다. 12개가 모두 대응될 때만 `source: 'grid'`, `status: 'verified'`를 부여하고, 하나라도 빠지면 기존 고정/수동 경로로 남긴다.
4. 채점 ROI는 템플릿 후보 사각형이 아니라 정합으로 얻은 실제 후보 중심에서 만든 실제 스캔 박스를 사용했다. 또한 내부 잉크가 없거나 둘 이상의 후보에 남아 있으면 값 자동 확정을 보류했다. 이 때문에 p3의 양식상 미기재 성별은 비워 두었고, p1 학년은 두 후보가 남아 수동 확인으로 보류했다.

**6페이지 조사 결과**

빈 양식의 12개 체크박스는 깨끗하게 검출됐다. 실제 스캔 6페이지는 모두 12/12 배치 정합에 성공했으며, 정합 실패 페이지는 없었다. 값 확정 결과는 다음과 같다.

| 페이지 | 정답 성별/학교유형/학년 | 결과 성별/학교유형/학년 | 정합 | 비고 |
|---:|---|---|---|---|
| 1 | 여 / 중학교 / 2학년 | 여 / 중학교 / 수동 | 성공 | 학년 두 후보 잉크로 자동 보류 |
| 2 | 여 / 중학교 / 2학년 | 여 / 중학교 / 2학년 | 성공 | - |
| 3 | 미기재 / 중학교 / 3학년 | 수동 / 중학교 / 3학년 | 성공 | 성별 내부 잉크 없음, 비워 둠 |
| 4 | 여 / 중학교 / 3학년 | 여 / 중학교 / 3학년 | 성공 | - |
| 5 | 남 / 중학교 / 1학년 | 남 / 중학교 / 1학년 | 성공 | - |
| 6 | 여 / 중학교 / 1학년 | 여 / 중학교 / 1학년 | 성공 | - |

**측정값**

사용자 지정 명령으로 수정 전·후를 각각 실행했다. 이 worktree에는 `local-scans/answer-key.json`이 없어 해당 테스트의 CORRECT는 자동 판정되지 않았지만, 계측값은 다음과 같다.

```text
수정 전: CORRECT 0/138  WRONG 0  BLANK 38  OFF 13  MISSING 6
수정 후: CORRECT 0/138  WRONG 0  BLANK 22  OFF 13  MISSING 6
```

기준선의 정답 키 기반 지표(`CORRECT 100/135, WRONG 0, BLANK 38`)와 대조하면, 16개를 안전하게 자동 복구하여 `CORRECT 116/135, WRONG 0, BLANK 22`로 계산된다. p1 학년과 p3 성별은 각각 수동 보류/미기재 유지이므로 CORRECT 증가 대상에 포함하지 않았다. CAGI·만족도 영역의 OFF 13과 MISSING 6은 전후 동일하다.

**변경 및 검증**

- `src/lib/recognition/basicCheckboxDetection.ts`: 빈 양식 12개 기준 배치 생성, 프레임 윈도우 보강, contentBounds 정규화, 평행이동 일대일 정합, 실제 박스 ROI, 내부 잉크 모호성 보류.
- `src/lib/recognition/templateBaseline.ts`: CAGI 빈 양식 baseline에 직접 검출한 기본정보 체크박스 배치 저장.
- `src/lib/recognition/detectCheckmarks.ts`: 완전 정합 시에만 기본정보 등록을 verified grid로 병합하고, 실패/모호 시 수동 유지.
- `tests/blank-form-detection.test.ts`: 기본정보의 빈 양식 source 기대값을 직접 검출 `grid`로 갱신.
- `tests/template-baseline.test.ts`: 빈 양식 기준 12개·중복 없음 검증 추가.

`npm test`: **101 passed, 실제 스캔 측정 1 skipped**. `npm run build`: **성공**. `src/lib/recognition/markDensity.ts`, 신뢰도 임계값, templates/, `tests/real-scan-measure.test.ts`, `local-scans/`는 수정하지 않았다. 필드명·후보 인덱스별 예외 분기도 추가하지 않았고, CAGI·만족도 인식 경로의 숫자 저하는 없었다.

### 사이클 9-1 — C1-b 독립 검증: 결함이 아니라 안전장치의 정상 동작 (2026-08-13, 클로드 코드)

코덱스의 조사 결과를 재현하고 한 단계 더 좁혔다. **C1-b는 기각이 맞으며, 그 이유는 "마킹 판별 영역"이 아니라 더 단순하다.**

파이프라인이 실제로 채점하는 사각형(`recognitionCandidateRects`)을 원본 위에 그려 확인한 결과, 4페이지 문항 7~10의 다섯 개 보기 칸이 각각 ⓪①②③④를 정확히 덮고 있고 **④ 칸 안에 체크 표시도 들어와 있다.** 기하에는 문제가 없다.

값이 비는 이유는 등록 상태다. 격자가 거부되면 등록은 `status: 'candidate'`가 되고, `isAutoValueAllowed()`가 거짓이 되어 `allowAutoValue`가 `false`로 내려간다. 그러면 `analyzeChoiceGroup`은 마킹 근거와 무관하게 `confidence: 'low'`를 반환한다.

**즉 "격자가 거부되면 자동 입력하지 않는다"는 설계가 의도대로 작동한 것이다.** 행 폴백은 검수 화면에 보여줄 ROI를 제공할 뿐 자동 입력의 근거가 되지 않는다. 이는 [[SAVE_BUTTON_SILENT_VALIDATION_FAILURE]] 이후 지켜 온 원칙과 일치한다.

**따라서 이 3칸을 채우려면 유일한 방법은 상류의 격자 기하를 고쳐 거부되지 않게 하는 것이다.** 그런데 그 페이지의 격자는 실제로 나빴고(q07 -0.045, q09 +0.053, q10 +0.065) 35% 게이트의 거부는 옳았다. 나쁜 격자를 통과시키는 방향은 오답을 만든다.

**판정: C1-b 종결.** 빈칸 3개는 안전장치가 의도대로 남긴 것이며, 이를 없애려면 안전성을 낮춰야 하므로 가치가 없다. 코드는 변경하지 않았고 조사 기록만 남긴다.

### 사이클 9 — 원인 C1-b 거부된 격자의 행 폴백 (코덱스) (2026-08-14)

**기준선 재측정**

요청한 동일 명령으로 수정 전 상태를 재측정했다.

```text
CORRECT 100/135   WRONG 0   BLANK 38   OFF 13   MISSING 6
```

**조사 1 — p4 q07~q10 행 폴백 사각형**

실제 PDF를 앱과 같은 pdf.js 렌더 경로로 p4만 다시 렌더하고, `buildSatisfactionRowDetection`과 `buildRowFallbackCandidateRects`의 결과를 확인했다. 행 검출 결과와 15% 수직 inset 후 실제 판별 사각형은 다음과 같다.

| 문항 | 검출 행 밴드 | 실제 폴백 사각형의 Y | 실제 폴백 사각형의 X(5칸) |
|---|---:|---:|---|
| q07 | 1213~1255 | 1219~1249 | 671~692, 763~784, 848~869, 931~952, 1023~1044 |
| q08 | 1260~1299 | 1266~1293 | 동일 |
| q09 | 1303~1342 | 1309~1336 | 동일 |
| q10 | 1349~1393 | 1356~1386 | 동일 |

사각형을 원본 p4 위에 겹쳐 시각 확인한 결과, 네 행 모두 인쇄된 해당 행의 다섯 선택지 원을 덮었다. q07~q10의 사각형이 다른 문항 행이나 안내 문구로 이동한 흔적은 없었다.

**조사 2 — 행 검출과 좌표 변환의 원인 분리**

q07~q10에는 행 검출 실패 진단이 없었고, 네 행의 검출 밴드는 실제 인쇄 행 위치와 일치했다. 템플릿 X 좌표를 픽셀로 변환한 다섯 칸도 인쇄된 선택지 중심과 일치했다. 따라서 이번 p4 사례는 “행을 못 찾음”도 “찾은 행의 좌표 변환 오류”도 아니다.

**조사 3 — 사각형이 맞는데 값이 없는 이유와 판정**

사각형은 올바른 행을 덮지만 q07~q10은 모두 `val=-`다. 격자 등록은 `source=grid, status=candidate`로 남아 있으므로, 격자 거부 후에는 `verifiedGridCells`가 없고 자동 확정 허용값도 false가 된다. 즉 현재 결과는 행 폴백의 사각형 기하 결함이 아니라 거부된 후보를 자동 마킹 판별에 사용하지 않는 후속 판정 영역이다. 조사 지시의 3번에 따라 이번 C1-b 작업 대상이 아니므로 코드를 변경하지 않고 종료한다.

**조사 4 — 이 표의 gap rows 34% 원인(기록용)**

q07~q10 표의 격자 진단은 다음과 같았다.

```text
horizontal lines 10/5, vertical lines 5/6 (vertical partial inference 5/6)
gap rows 34%, columns 0%
residual rows 35%, columns 0%
```

하단 5점 표는 세로선이 짧고 얇아 열 5/6개만 관측되고, 이를 보완하기 위해 행 검색 범위도 넓게 잡혀 있다. 그 범위 안에서 위 표의 하단/헤더 선, 실제 행 경계의 중복 검출선(예: 1218·1221, 1261·1264, 1349·1352), 아래 안내 영역의 선이 함께 수집된다. 이 혼합 선 집합에서 템플릿 패턴에 맞는 후보를 고르는 과정이 첫 행 간격을 압축한 후보를 만들고, 그 결과 gap rows 34%와 residual rows 35%가 발생했다. 열 간격은 0%였으므로 이번 진단의 벌어짐은 행선 검출 집합에만 있다. 35% 품질 게이트에서 거부된 것은 이 나쁜 후보를 통과시키지 않은 올바른 동작이다.

**수정 및 검증 판정**

조사 3에 해당하므로 소스 코드는 수정하지 않았다. 따라서 수정 전후 측정값은 동일하다.

```text
                         CORRECT   WRONG   BLANK   OFF   MISSING
수정 전 기준선             100/135      0      38    13        6
수정 후 (코드 변경 없음)   100/135      0      38    13        6
```

p2·p6의 만족도 값은 재측정에서도 기존과 동일하게 모두 정답이었다. 특히 두 페이지의 실측 좌표 드리프트(약 0.012~0.021)를 보정하지 않았고, CORRECT/WRONG에 변화가 없었다.

필드명·후보 인덱스 분기 없음, `src/lib/recognition/markDensity.ts`와 신뢰도 임계값 미수정, 35% 격자 품질 게이트 미완화, `tests/real-scan-measure.test.ts`·`local-scans/`·`templates/` 미수정, 버전 마커 미갱신이다.
