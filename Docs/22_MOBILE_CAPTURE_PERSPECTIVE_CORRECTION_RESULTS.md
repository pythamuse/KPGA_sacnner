...
실기기 테스트에서 "선별검사지 촬영하기" 버튼이 동작하지 않는 것처럼 보인다는 제보가 있었다. 원인을 확인한 결과 코드가 멈춘 게 아니라 **피드백이 없었던 것**이었다:

- opencv.js는 실제로 **약 10.2MB**(`curl`로 직접 확인)이다. `captureCurrentFrame`이 이 파일을 로드하는 동안(최대 5초 타임아웃, 모바일 회선에서는 그 이상 걸릴 수도 있음) 버튼은 비활성화되지도, 라벨이 바뀌지도 않아 사용자 입장에서는 탭했는데 아무 반응이 없는 것처럼 보였다. 실제 업로드가 시작되는 시점(`isCagiUploading`/`isSatUploading`)에만 "업로드 중" 표시가 있었고, 그 앞의 OpenCV 로드+분석 구간에는 표시가 전혀 없었다.

`src/components/ImageUploadPanel.tsx`에 `isCapturing` 상태를 추가해 버튼을 누른 즉시(캔버스 캡처 시작 시점부터 함수 종료까지, OpenCV 로드/분석 구간 포함) 버튼을 비활성화하고 "처리 중"으로 라벨을 바꾸도록 수정했다. "촬영 취소" 버튼도 같은 구간에는 비활성화해 캡처 도중 카메라 세션이 끊기는 경쟁 상태를 방지했다. 이 수정은 규모가 작고 원인이 명확해 Codex를 거치지 않고 직접 수정했다.
