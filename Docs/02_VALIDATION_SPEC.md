...
## 8. 성인 CPGI 트랙 검증 규칙 (2026-08-01 추가)

`student.track === 'adult'`이면 다음 규칙으로 대체한다. `track` 미지정 시 기존 청소년 규칙(위 1~6장)을 그대로 적용한다.

### 8.1 기본 정보

| 필드 | 필수 여부 | 허용값 | 비고 |
|---|---:|---|---|
| age | 필수 | 20, 30, 40, 50, 60, 70 | 실제 나이가 아니라 연령대 코드. `normalizeAdultAgeBand()`가 10 단위로 내림 보정 |
| gender | 필수 | 남, 여 | 청소년과 동일 |
| schoolType | 검증 안 함 | - | 성인 템플릿에 해당 컬럼이 없음 |
| grade | 검증 안 함 | - | 성인 템플릿에 해당 컬럼이 없음 |

오류 코드: `INVALID_AGE_BAND`.

### 8.2 CPGI 문항

CAGI와 동일하게 9개 문항 모두 0~3만 허용한다(오류 코드도 동일하게 `INVALID_CAGI_VALUE` 재사용). 원본 템플릿의 `x14:dataValidation`이 C3:K 전체에 `코드!$C$2:$C$5`(0~3)만 참조하므로 문항별로 다른 허용값이 없다.

### 8.3 성인 만족도 문항

청소년과 달리 문항1~10 전부 0~4 균일 척도다(원본 템플릿 `x14:dataValidation` sqref가 C3:L 전체에 `코드!$D$2:$D$6`만 참조). 문항1을 1~4로, 문항2~6을 0/1로 제한하는 청소년 규칙을 성인 데이터에 적용하면 안 된다.

| 필드 | 필수 여부 | 허용값 |
|---|---:|---|
| satisfaction.q01~q10 | 필수 | 0, 1, 2, 3, 4 |

오류 코드: `INVALID_SAT_SCALE` (청소년의 5점 척도 문항과 동일 코드 재사용).

### 8.4 저장 가능 조건

이미지 존재 여부, 사용자 확인 여부 등 나머지 저장 조건(5장)은 트랙과 무관하게 동일하다.
